...
xcodebuild -workspace xebia-ios.xcworkspace -scheme "xebia-ios" -sdk iphonesimulator7.0 -configuration Debug clean build OBJROOT="$PWD/build" SYMROOT="$PWD/build" PROJECT_TEMP_ROOT="$PWD/build" CONFIGURATION_BUILD_DIR="$PWD/build"
...
xcodebuild -workspace xebia-ios.xcworkspace -scheme "xebia-iosTests" -sdk iphonesimulator7.0 -configuration Debug clean build OBJROOT="$PWD/build" SYMROOT="$PWD/build" PROJECT_TEMP_ROOT="$PWD/build" CONFIGURATION_BUILD_DIR="$PWD/build" TEST_AFTER_BUILD=YES SL_RUN_UNIT_TESTS=YES
ELAPSED_TIME=$(($SECONDS - $START_TIME))
...
START_TIME=$SECONDS
xcodebuild -workspace xebia-ios.xcworkspace -scheme "xebia-ios" -sdk iphoneos7.0 -configuration Release clean build OBJROOT="$PWD/build" SYMROOT="$PWD/build" PROJECT_TEMP_ROOT="$PWD/build" CONFIGURATION_BUILD_DIR="$PWD/build" CODE_SIGN_IDENTITY="${DEVELOPER_NAME}" PROVISIONING_PROFILE="${PROFILE_UUID}" | awk '{print substr($0,0,220)}'
ELAPSED_TIME=$(($SECONDS - $START_TIME))
