...
@implementation XECardDetailsFullContentPageViewController

// load the view nib and initialize the pageNumber ivar
    - (id)init
    {

        if (self = [self.appDelegate.viewControllerManager getOrCreateControllerWithIdentifier:@"cardDetailsFullContentPage"]) {
    ...
    - (void)viewDidLoad {
        ...
        self.fullContentTextView.backgroundColor = [UIColor clearColor];

        // we draw images and links via subviews provided by delegate methods
        self.fullContentTextView.shouldDrawImages = NO;
        self.fullContentTextView.shouldDrawLinks = NO;
        self.fullContentTextView.textDelegate = self; // delegate for custom sub views

        // gesture for testing cursor positions
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTap:)];
        [self.fullContentTextView addGestureRecognizer:tap];

        // set an inset. Since the bottom is below a toolbar inset by 44px
        [self.fullContentTextView setScrollIndicatorInsets:UIEdgeInsetsMake(0, 0, 44, 0)];
    }

    -(void)viewWillAppear:(BOOL)animated {
        [super viewWillAppear:animated];
        if (self.card) {
            self.fullContentTextView.attributedString = [self attributedStringForCard];
        }
    }
    ...
    {
        // Load HTML data
        NSData *data = [self.card.fullContent dataUsingEncoding:NSUTF8StringEncoding];
    ...
    - (void)handleTap:(UITapGestureRecognizer *)gesture
    {
        ...
        {
            CGPoint location = [gesture locationInView:self.fullContentTextView];
            NSUInteger tappedIndex = (NSUInteger)[self.fullContentTextView closestCursorIndexToPoint:location];

            NSString *plainText = [self.fullContentTextView.attributedString string];
